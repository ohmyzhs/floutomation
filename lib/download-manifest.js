...
// Media cards link the asset serving endpoint with a display directive, e.g.
// ".../asb/<token>=s1600-rw". "-rw" asks for WebP and "s1600" for a resize;
// "=s0" is the stored original, which Flow serves as JPEG. Both hosts carry
// the same token; flow.google.com is the one whose session we hold.
const ASSET_SERVING_HOSTS = [
  "https://flow.google.com/asb/",
  "https://lh3.googleusercontent.com/asb/"
];

export function flowOriginalUrl(value) {
  const url = String(value || "");
  const prefix = ASSET_SERVING_HOSTS.find((host) => url.startsWith(host));
  if (!prefix) return url;
  const token = url.slice(prefix.length).split("=", 1)[0];
  if (!token) return url;
  return `https://flow.google.com/asb/${token}=s0`;
}
