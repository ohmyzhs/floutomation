...
const LH3_ASSET_PREFIX = "https://lh3.googleusercontent.com/asb/";

// Media cards carry a preview link. Its token is served at full size by
// flow.google.com/asb/<token>=s0 — the stored original, as JPEG — while the
// bare lh3 link bounces through a host whose session we do not carry.
export function flowOriginalUrl(value) {
  const url = String(value || "");
  if (!url.startsWith(LH3_ASSET_PREFIX)) return url;
  const token = url.slice(LH3_ASSET_PREFIX.length);
  if (!token) return url;
  // Drop whatever size directive the preview asked for, then request the
  // original. A bare token would otherwise serve a 512px preview.
  const original = `${token.split("=", 1)[0]}=s0`;
  return `https://flow.google.com/asb/${original}`;
}
