...
const INTRO_SCENE_PATTERN = /^\s*={5,}\s*씬\s*(\d+)\s*={5,}.*$/i;
// Intro-hook files are commonly exported with Markdown-style decorations,
// e.g. `--- [A] 이미지 프롬프트 ---`, while older files use the bare marker.
// Accept both forms so the marker itself is not mistaken for missing content.
const INTRO_IMAGE_PROMPT_PATTERN = /^\s*(?:[-=*]{3,}\s*)?\[A\]\s*이미지\s*프롬프트(?:\s*\([^)]*\))?\s*[:：-]?\s*(?:[-=*]{3,})?\s*$/i;
const INTRO_BLOCK_MARKER_PATTERN = /^\s*(?:[-=*]{3,}\s*)?\[[^\]]+\]/i;
...
function introPromptBlock(lines, startIndex, endIndex) {
  ...
  for (; cursor < endIndex; cursor += 1) {
    ...
    if (!line.trim() || INTRO_BLOCK_MARKER_PATTERN.test(line) || INTRO_SCENE_PATTERN.test(line)) break;
...
export function analyzePrompts(rawText, { mode = "scene", knownCharacterKeys = [] } = {}) {
  ...
  const flowSignals = /\[영어\s*프롬프트|===\s*[A-Za-z][\w-]*\s+UPLOAD|===\s*프롬프트/i.test(text);
  const flowWarning = flowSignals
    ? "Flow_Prompts 형식의 장면·캐릭터 구분을 완성하지 못했습니다. [영어 프롬프트 N~N], === 캐릭터 (설명) ===, === 캐릭터 UPLOAD === 구조를 확인하세요."
    : "";

  const headingSections = parseHeadingSections(text);
  if (headingSections) return finalize(headingSections, "markdown-headings", flowWarning ? [flowWarning] : []);

  const labeledSections = parseLabeledSections(text);
  if (labeledSections) return finalize(labeledSections, "prompt-labels", flowWarning ? [flowWarning] : []);

  const listItems = parseList(text);
  if (listItems) return finalize(listItems, "markdown-list", flowWarning ? [flowWarning] : []);

  const delimited = splitDelimiters(text);
  if (delimited) return finalize(delimited, "delimiters", flowWarning ? [flowWarning] : []);

  const blankBlocks = splitBlankBlocks(text);
  if (blankBlocks) return finalize(blankBlocks, "blank-lines", flowWarning ? [flowWarning] : []);

  const lines = text.split("\n").map(normalizePrompt).filter(Boolean);
  if (lines.length >= 2) {
    return finalize(lines, "lines", [
      ...(flowWarning ? [flowWarning] : []),
      "명시적인 구분자를 찾지 못해 각 줄을 하나의 이미지 프롬프트로 해석했습니다."
    ]);
  }

  return finalize([text], "single", [
    ...(flowWarning ? [flowWarning] : []),
    "프롬프트 구분자를 찾지 못해 전체 내용을 1개 작업으로 인식했습니다. 장면별 프롬프트를 빈 줄, 번호 목록 또는 ## 제목으로 나누세요."
  ]);
}
