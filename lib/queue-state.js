...
export function normalizeOptions(value = {}) {
  ...
  return {
    ...
    imagesPerPrompt: DEFAULT_OPTIONS.imagesPerPrompt,
    delayMs: Math.max(MIN_DELAY_MS, Number.isFinite(requestedDelay) ? requestedDelay : MIN_DELAY_MS),
...
export function hydrateState(value) {
  ...
  const jobs = Array.isArray(value.jobs)
    ? value.jobs.map((job) => ({ ...job, resultAssets: Array.isArray(job.resultAssets) ? job.resultAssets : [] }))
    : [];
...
export function summarizeState(state) {
  ...
  return {
    ...
    charactersCompleted: characters.filter((character) => character.status === "completed").length,
...
export function carryForwardCompletedTasks(nextState, previousState) {
  ...
  const previousJobs = new Map(
    (previousState.jobs || [])
      ...
      .map((job) => [`${Number(job.number || 0)}\u0000${String(job.prompt || "").trim()}`, job])
  );
  for (const job of nextState.jobs || []) {
    const previous = previousJobs.get(`${Number(job.number || 0)}\u0000${String(job.prompt || "").trim()}`);
    if (!previous) continue;
    job.status = "completed";
