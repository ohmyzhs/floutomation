...
function normalizeMappingJobs(value) {
  ...
  return (Array.isArray(value) ? value : []).flatMap((job) => {
    ...
    const characterRefs = uniqueStrings(job?.characterRefs);
    // A scene that has not produced an image yet is still part of the
    // project's roster. Dropping it here is what left "연결하기" with an
    // empty scene list whenever generation had been failing.
    if (!assets.length && !mappedAssetIds.length && !title && !characterRefs.length) return [];
    seen.add(key);
    return [{
      sourceMode,
      sourceNumber,
...
export function isFlowUrl(value) {
  ...
  return url.hostname === "flow.google.com" && DIRECT_FLOW_PATH.test(url.pathname);
}
...
export function characterDetailFromFlowUrl(value) {
  const url = flowUrl(value);
  if (!url || !isFlowUrl(url.href)) return null;
  const match = url.pathname.match(/^\/project\/([^/?#]+)\/character\/([^/?#]+)(?:\/|$)/i);
  if (!match) return null;
  const projectId = decodeURIComponent(match[1]).trim();
  const characterId = decodeURIComponent(match[2]).trim();
